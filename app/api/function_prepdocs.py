...
class PrepDocsManager:
    ...
    def get_document_text(self, filename, localpdfparser):
        ...
        if localpdfparser:
            ...
            form_recognizer_client = DocumentAnalysisClient(endpoint, self.credential, headers={"x-ms-useragent": "azure-search-chat-demo/1.0.0"})
    ...
    def create_sections(self, filename, page_map, use_vectors):        
        ...
        for i, (content, page_num) in enumerate(self.split_text(filename, page_map)):
            section = {"id": f"{file_id}-page-{i}", "content": content, "category": "", "sourcepage": self.blob_name_from_file_page(filename, page_num), "sourcefile": filename}
            if use_vectors:
